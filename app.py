...
def get_cached_ratios_data() -> pd.DataFrame:
    ...
    df["year_int"] = df["year"].apply(lambda y: int(y[-4:]) if y and len(str(y)) >= 4 and y[-4:].isdigit() else 2024)
...
# ----------------------------------------------------
st.sidebar.markdown("<h2 style='text-align: center; color: #38BDF8;'>⚡ N100 Engine</h2>", unsafe_allow_html=True)
...
menu_selection = st.sidebar.radio(
    "Select View",
    ["Executive Overview", "Interactive Stock Matcher", "Predefined Screeners", 
     "Sector Analysis", "AI-Powered Research", "Custom Portfolio Risk", "3-Year Projections"]
)
...
def get_watchlist() -> list[str]:
    ...
    conn = get_db_connection()
    try:
        rows = conn.execute("SELECT company_id FROM watchlists WHERE user_id = ?", (st.session_state.user_id,)).fetchall()
...
if menu_selection == "Executive Overview":
    ...
    st.markdown("<p style='color: #94A3B8;'>Real-time metrics, sector distributions, and correlation maps across Nifty 100 universe.</p>", unsafe_allow_html=True)
    ...
    with c1:
        ...
        fig_tree.update_layout(margin=dict(t=10, l=10, r=10, b=10), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
    ...
    with c2:
        ...
        fig_bubble.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#E2E8F0")
        st.plotly_chart(fig_bubble, use_container_width=True)

    # Correlation Heatmap
    st.subheader("Ratio Multi-Correlation Map")
    kpis = ["ROE", "ROCE", "Revenue CAGR", "PAT CAGR", "Debt to Equity", "Operating Margin", "Interest Coverage", "Composite Quality Score"]
    ...
    fig_heat.update_layout(margin=dict(t=10, l=10, r=10, b=10), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#E2E8F0")
    st.plotly_chart(fig_heat, use_container_width=True)


# Page 2: Interactive Stock Matcher
elif menu_selection == "Interactive Stock Matcher":
    st.markdown("<h1 class='main-title'>⚡ Side-by-Side Comparison</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: #94A3B8;'>Perform head-to-head financial analysis across two companies.</p>", unsafe_allow_html=True)
    ...
    with col2:
        comp_b = st.selectbox("Select Company B", comp_list, index=min(1, len(comp_list)-1))
    ...
    for display_name, col_name in kpi_map.items():
        ...
        if isinstance(val_a, (int, float)) and isinstance(val_b, (int, float)):
            if col_name == "Debt to Equity":  # Lower is better
                if val_a < val_b: better_marker_a = " 🟢"
                elif val_b < val_a: better_marker_b = " 🟢"
    ...
    with c1:
        # ROE Trend chart
        fig_roe = go.Figure()
        fig_roe.add_trace(go.Scatter(x=hist_a["year"], y=hist_a["ROE"], name=f"{comp_a} ROE", line=dict(color="#38BDF8", width=3)))
        fig_roe.add_trace(go.Scatter(x=hist_b["year"], y=hist_b["ROE"], name=f"{comp_b} ROE", line=dict(color="#C084FC", width=3)))
        fig_roe.update_layout(title="Historical ROE Trend", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#E2E8F0")
    ...
    with c2:
        ...
        conn = get_db_connection()
        sales_a = pd.read_sql("SELECT year, sales FROM profitandloss WHERE company_id = ? ORDER BY year", conn, params=[comp_a])
        sales_b = pd.read_sql("SELECT year, sales FROM profitandloss WHERE company_id = ? ORDER BY year", conn, params=[comp_b])
        conn.close()
        
        fig_sales.add_trace(go.Scatter(x=sales_a["year"], y=sales_a["sales"], name=f"{comp_a} Sales (Cr)", line=dict(color="#10B981", width=3)))
        fig_sales.add_trace(go.Scatter(x=sales_b["year"], y=sales_b["sales"], name=f"{comp_b} Sales (Cr)", line=dict(color="#F59E0B", width=3)))
        fig_sales.update_layout(title="Historical Sales Revenue", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#E2E8F0")
        st.plotly_chart(fig_sales, use_container_width=True)


# Page 3: Predefined Screeners
elif menu_selection == "Predefined Screeners":
    st.markdown("<h1 class='main-title'>⚡ Predefined Analyst Screeners</h1>", unsafe_allow_html=True)
...
# Page 4: Sector Analysis
    ...
    st.markdown("<h1 class='main-title'>⚡ Sector-wise Peer ranking</h1>", unsafe_allow_html=True)
    
    from src.peer_analysis.comparison import run_peer_analysis
    peer_comp_df, sector_stats_df, top_perf_df, bottom_perf_df = run_peer_analysis(DB_PATH)
    ...
    sector_group = peer_comp_df[peer_comp_df["Sector"] == selected_sector].sort_values(by="Peer Rank")
...
# Page 5: AI-Powered Research
    ...
    st.markdown("<h1 class='main-title'>⚡ AI Copilot Insights</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: #94A3B8;'>Instant, rule-based natural language summaries and automated PDF reporting.</p>", unsafe_allow_html=True)
...
# Page 6: Custom Portfolio Risk
    ...
    st.markdown("<h1 class='main-title'>⚡ Portfolio Allocation Builder</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: #94A3B8;'>Input investment and allocate capital across chosen Nifty 100 securities to evaluate overall risk indicators.</p>", unsafe_allow_html=True)
    
    capital = st.number_input("Total Investment Capital (₹)", min_value=1000, value=100000, step=5000)
    
    comp_list = sorted(df_latest["Company"].tolist())
    selected_assets = st.multiselect("Select Portfolio Assets", comp_list, default=["TCS", "INFY"] if "TCS" in comp_list and "INFY" in comp_list else comp_list[:2])
    ...
    if len(selected_assets) < 1:
        ...
        for idx, asset in enumerate(selected_assets):
            with cols[idx]:
                alloc_pct = st.slider(f"{asset} Weight", 0, 100, int(100 / len(selected_assets)))
        ...
        if total_alloc != 100:
            ...
            st.metric("Diversification Score", f"{metrics['diversification_score']} / 100")
...
# Page 7: 3-Year Projections
    ...
    st.markdown("<h1 class='main-title'>⚡ Predictive Financial Projections</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: #94A3B8;'>3-Year forecasts for Sales Revenue and Earnings Per Share (EPS) using Linear Regression and Moving Average curves.</p>", unsafe_allow_html=True)
    ...
    if fc_res.get("success", False):
        ...
        with c1:
            ...
            fig_sales = go.Figure()
            # Historical
            fig_sales.add_trace(go.Scatter(x=hist_df["Year"], y=hist_df["Revenue"], mode="markers+lines", name="Historical", line=dict(color="#10B981", width=3)))
            # Linear Regression
            fig_sales.add_trace(go.Scatter(
                x=[hist_df["Year"].iloc[-1]] + list(fc_df["Year"]), 
                y=[hist_df["Revenue"].iloc[-1]] + list(fc_df["Revenue (Linear Regression)"]), 
                mode="lines", name="Linear Trend Forecast", line=dict(color="#38BDF8", dash="dash", width=3)
            ))
            # Moving Average
            fig_sales.add_trace(go.Scatter(
                x=[hist_df["Year"].iloc[-1]] + list(fc_df["Year"]), 
                y=[hist_df["Revenue"].iloc[-1]] + list(fc_df["Revenue (Moving Average)"]), 
                mode="lines", name="Moving Average Forecast", line=dict(color="#F59E0B", dash="dot", width=3)
            ))
            fig_sales.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#E2E8F0")
            st.plotly_chart(fig_sales, use_container_width=True)
            st.write(f"Compound growth rate based on trend slope: **{fc_res['revenue_trend_growth_rate']}%**")
            
        with c2:
            st.subheader("Earnings Per Share (EPS) Projections")
            fig_eps = go.Figure()
            # Historical
            fig_eps.add_trace(go.Scatter(x=hist_df["Year"], y=hist_df["EPS"], mode="markers+lines", name="Historical", line=dict(color="#C084FC", width=3)))
            # Linear Regression
            fig_eps.add_trace(go.Scatter(
                x=[hist_df["Year"].iloc[-1]] + list(fc_df["Year"]), 
                y=[hist_df["EPS"].iloc[-1]] + list(fc_df["EPS (Linear Regression)"]), 
                mode="lines", name="Linear Trend Forecast", line=dict(color="#E879F9", dash="dash", width=3)
            ))
            # Moving Average
            fig_eps.add_trace(go.Scatter(
                x=[hist_df["Year"].iloc[-1]] + list(fc_df["Year"]), 
                y=[hist_df["EPS"].iloc[-1]] + list(fc_df["EPS (Moving Average)"]), 
                mode="lines", name="Moving Average Forecast", line=dict(color="#F43F5E", dash="dot", width=3)
            ))
            fig_eps.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#E2E8F0")
            st.plotly_chart(fig_eps, use_container_width=True)
            st.write(f"Compound growth rate based on trend slope: **{fc_res['eps_trend_growth_rate']}%**")
