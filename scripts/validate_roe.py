...
def _load_latest_computed_roe(db_path: Path) -> pd.DataFrame:
    if db_path.exists():
        conn = sqlite3.connect(db_path)
        computed_df = pd.read_sql_query(
            "SELECT company_id, year, return_on_equity_pct AS computed_value FROM financial_ratios",
            conn,
        )
        conn.close()
...
def main() -> None:
    ...
    computed_df = _load_latest_computed_roe(db_path)

    mis_matches = build_ratio_mismatch_entries(
        computed_df,
        companies_df.rename(columns={"roe_percentage": "source"}),
        ratio_name="ROE",
