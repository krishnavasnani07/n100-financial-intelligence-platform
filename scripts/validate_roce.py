...
def _load_companies_source(path: Path) -> pd.DataFrame:
    ...
    companies_df["roce_percentage"] = pd.to_numeric(
        companies_df["roce_percentage"], errors="coerce"
    )
    return companies_df
...
def _load_latest_computed_roce(db_path: Path) -> pd.DataFrame:
    ...
    computed_df = financial_ratios_df[
        ["company_id", "return_on_capital_employed_pct"]
    ].copy()
