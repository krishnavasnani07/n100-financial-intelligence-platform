...
def test_winsorize_and_scale_higher_is_better():
    ...
    assert scores.iloc[0] == 0.0      # Outlier 0.0 winsorized to p10 (1.0), which scales to 0.0
    assert scores.iloc[1] == 0.0      # 1.0 is p10, scales to 0.0
    assert scores.iloc[10] == 100.0   # Outlier 10.0 winsorized to p90 (9.0), which scales to 100.0
...
def test_calculate_rankings_execution():
    ...
    required_cols = [
        "company_id", "overall_rank", "composite_quality_score",
        "fcf_cagr_5yr", "cfo_pat_ratio", "fcf_positive"
    ]
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT composite_quality_score FROM financial_ratios WHERE year = ?", (df_ranked["year"].iloc[0],))
...
def test_excel_report_generation():
    ...
    expected_sheets = [
        "Summary", "Rankings", "Quality Compounder", "Value Pick",
        "Growth Accelerator", "Dividend Champion", "Debt-Free Blue Chip", "Turnaround Watch"
    ]
    ...
    expected_csvs = [
        "rankings.csv", "quality_compounder.csv", "value_pick.csv",
        "growth_accelerator.csv", "dividend_champion.csv", "debt_free_blue_chip.csv", "turnaround_watch.csv"
    ]
