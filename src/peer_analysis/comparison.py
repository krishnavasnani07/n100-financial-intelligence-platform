...
def get_raw_company_ratios(company_id: str, db_path: Optional[Path] = None) -> Dict[str, Any]:
...
def determine_winner(kpi: str, val_a: Any, val_b: Any, company_a: str, company_b: str) -> str:
...
def generate_executive_summary(company_a: str, company_b: str, a_data: Dict[str, Any], b_data: Dict[str, Any]) -> str:
    ...
    for kpi in ["Revenue CAGR", "PAT CAGR"]:
        w = determine_winner(kpi, a_data.get(kpi), b_data.get(kpi), company_a, company_b)
        if w == company_a:
            a_growth_wins += 1
        elif w == company_b:
            b_growth_wins += 1
    ...
    if a_growth_wins > b_growth_wins:
        summary_parts.append(f"{company_a} demonstrates superior performance in growth metrics (Revenue & PAT CAGRs).")
    elif b_growth_wins > a_growth_wins:
        summary_parts.append(f"{company_b} demonstrates superior performance in growth metrics (Revenue & PAT CAGRs).")
    else:
    ...
    if a_profit_wins > b_profit_wins:
        summary_parts.append(f"In terms of profitability, {company_a} leads in operational efficiency and returns on equity/capital.")
    elif b_profit_wins > a_profit_wins:
        summary_parts.append(f"In terms of profitability, {company_b} leads in operational efficiency and returns on equity/capital.")
    else:
        summary_parts.append("They exhibit comparable operational profitability profiles.")
        
    return " ".join(summary_parts)


def generate_peer_comparison_report(company_a: str, company_b: str, db_path: Optional[Path] = None) -> Tuple[Path, Path]:
    """
    ...
    try:
        a_rank = int(peer_comp[peer_comp["Company"] == company_a]["Peer Rank"].values[0])
    ...
    try:
        b_rank = int(peer_comp[peer_comp["Company"] == company_b]["Peer Rank"].values[0])
    ...
    kpis = [
        ...
        ("Revenue CAGR (%)", a_data.get("Revenue CAGR"), b_data.get("Revenue CAGR"), "0.1f"),
        ("PAT CAGR (%)", a_data.get("PAT CAGR"), b_data.get("PAT CAGR"), "0.1f"),
        ("Operating Margin (%)", a_data.get("Operating Margin"), b_data.get("Operating Margin"), "0.1f"),
        ("Debt to Equity", a_data.get("Debt to Equity"), b_data.get("Debt to Equity"), "0.02f"),
        ("Interest Coverage", a_data.get("Interest Coverage"), b_data.get("Interest Coverage"), "0.02f"),
        ("Composite Score", a_data.get("Composite Quality Score"), b_data.get("Composite Quality Score"), "0.1f"),
    ...
    ws.cell(row=2, column=1, value=f"Target: {company_a} vs {company_b} ({sector_label})").font = italic_font
    ws.cell(row=3, column=1, value=f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}").font = italic_font
    ...
    for r_idx, (kpi, val_a, val_b, fmt) in enumerate(kpis):
        ...
        # Winner calculation
        win_company = determine_winner(kpi.replace(" (%)", ""), val_a, val_b, company_a, company_b)
    ...
    story.append(Paragraph(f"PEER COMPARISON REPORT: {company_a} VS {company_b}", title_style))
    story.append(Paragraph(f"Sector Universe: {sector_label}  |  Generated on: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}", subtitle_style))
    ...
    # Table Header Row
    header_row = [
        Paragraph("<b>KPI Metric</b>", ParagraphStyle('H1', parent=cell_bold_style, textColor=colors.white)),
        Paragraph(f"<b>{company_a}</b>", ParagraphStyle('H2', parent=cell_bold_style, textColor=colors.white)),
        Paragraph(f"<b>{company_b}</b>", ParagraphStyle('H3', parent=cell_bold_style, textColor=colors.white)),
        Paragraph("<b>Winner</b>", ParagraphStyle('H4', parent=cell_bold_style, textColor=colors.white))
    ]
    ...
    for r_idx, (kpi, val_a, val_b, fmt) in enumerate(kpis):
        ...
        str_a = f"{val_a:.1f}%" if "1f" in fmt and kpi != "Sector" else (f"{val_a:.2f}" if "2f" in fmt else str(val_a))
        str_b = f"{val_b:.1f}%" if "1f" in fmt and kpi != "Sector" else (f"{val_b:.2f}" if "2f" in fmt else str(val_b))
