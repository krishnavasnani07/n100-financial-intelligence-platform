...
def calculate_normalized_metrics(df_universe: pd.DataFrame) -> pd.DataFrame:
    ...
    for metric, lower_is_better in metrics_config.items():
        ...
        if p90 > p10:
            ...
            scores = pd.Series(100.0 if not lower_is_better else 0.0, index=df_universe.index)
...
def get_company_metrics(company_id: str, df_norm: pd.DataFrame, df_raw: pd.DataFrame) -> Tuple[List[float], List[float]]:
    ...
    metrics_list = ["roe", "roce", "revenue_cagr", "pat_cagr", "operating_margin", "current_ratio", "debt_to_equity", "composite_score"]
    ...
    r_vals = [float(raw_row.iloc[0][m]) if pd.notnull(raw_row.iloc[0][m]) else 0.0 for m in metrics_list]
    
    return n_vals, r_vals


def generate_single_radar(company_id: str, save_path: Optional[Path] = None, db_path: Optional[Path] = None) -> Path:
    """
    ...
    categories = ['ROE', 'ROCE', 'Revenue CAGR', 'PAT CAGR', 'Operating Margin', 'Current Ratio', 'Debt to Equity', 'Composite Score']
    N = len(categories)
    ...
    # Plot company data
    ax.plot(angles, plot_vals, color='#1B365D', linewidth=2, linestyle='solid', label=company_id)
    ax.fill(angles, plot_vals, color='#1B365D', alpha=0.15)
    
    # Chart Title
    plt.title(f"Financial Health & Quality Profile: {company_id}", size=14, color='#1B365D', fontweight='bold', pad=25)
    
    # Save chart
    out_file = save_path or (OUTPUT_DIR / "charts" / "radar" / f"{company_id}_radar.png")
...
def generate_peer_radar(company_a: str, company_b: str, save_path: Optional[Path] = None, db_path: Optional[Path] = None) -> Path:
    """
    ...
    ax.plot(angles, a_plot_vals, color='#1B365D', linewidth=2, linestyle='solid', label=company_a)
    ax.fill(angles, a_plot_vals, color='#1B365D', alpha=0.15)
    
    # Plot Company B (Warm Amber/Coral)
    ax.plot(angles, b_plot_vals, color='#D9534F', linewidth=2, linestyle='solid', label=company_b)
    ax.fill(angles, b_plot_vals, color='#D9534F', alpha=0.15)
    
    # Add Title and Legend
    plt.title(f"Peer Quality Comparison: {company_a} vs {company_b}", size=14, color='#1B365D', fontweight='bold', pad=25)
    plt.legend(loc='upper right', bbox_to_anchor=(1.1, 1.1), frameon=True, facecolor='white', edgecolor='#E5E5E5')
    
    # Save chart
    out_file = save_path or (OUTPUT_DIR / "charts" / "peer" / f"{company_a}_vs_{company_b}_radar.png")
    out_file.parent.mkdir(parents=True, exist_ok=True)
