...
def generate_trend_charts(
    ...
    fig.suptitle(
        f"Historical Financial Trends: {company_id}",
        fontsize=16,
        fontweight="bold",
        color="#1B365D",
        y=0.98,
    ...
    ax3.set_title(
        "Return on Equity (%)", fontsize=12, fontweight="bold", color="#333333"
    )
    ...
    ax4.set_title(
        "Operating Margin Trend (%)", fontsize=12, fontweight="bold", color="#333333"
    )
    ...
    out_file = save_path or (
        OUTPUT_DIR / "charts" / "trends" / f"{company_id}_trends.png"
    )
