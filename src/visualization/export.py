...
from src.visualization.radar_chart import generate_single_radar, generate_peer_radar
...
def export_all_charts(db_path: Optional[Path] = None, output_dir: Optional[Path] = None) -> None:
    """
    ...
    logger.info(f"Loaded {len(sectors)} sectors and {len(companies)} companies from database.")
    ...
    for sector in sectors:
        try:
            sec_filename = sector.lower().replace(" ", "_").replace("-", "_") + "_heatmap.png"
