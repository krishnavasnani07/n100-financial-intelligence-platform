...
def load_historical_pl(company_id: str, db_path: Optional[Path] = None) -> pd.DataFrame:
    ...
    df = df.dropna(subset=["year_int"]).sort_values(by="year_int").reset_index(drop=True)
    return df
...
def forecast_metric(
    ...
    if n < 2:
        ...
        next_years = np.arange(years[-1] + 1, years[-1] + 1 + forecast_years_ahead) if n > 0 else np.array([2025, 2026, 2027])
...
def generate_company_forecasts(company_id: str, db_path: Optional[Path] = None) -> Dict[str, any]:
    """
