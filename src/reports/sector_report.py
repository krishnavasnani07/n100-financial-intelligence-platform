...
from src.reports.styles import (
    NAVY_PRIMARY, GOLD_ACCENT, GREY_SLATE, BORDER_LIGHT, PRO_GREEN, CON_RED,
    TEXT_NEUTRAL, TEXT_LIGHT, PRINTABLE_WIDTH,
    title_style, subtitle_style, meta_style, meta_subtitle_style,
    section_heading_style, kpi_label_style, kpi_value_style, kpi_unit_style,
    table_header_style, table_cell_style, table_cell_bold_style
)
from src.reports.layouts import NumberedCanvas
from src.reports.report_utils import map_sector, check_eligibility
...
def make_sector_kpi_card(label: str, val_str: str, unit: str, color_hex: str) -> list:
    ...
    value_p = Paragraph(f"<font color='{color_hex}'><b>{val_str}</b></font>", kpi_value_style)
    unit_p = Paragraph(unit, kpi_unit_style)
    return [label_p, Spacer(1, 4), value_p, Spacer(1, 2), unit_p]


def build_sector_header(sector_name: str, company_count: int, latest_year: str) -> Table:
...
def generate_sector_pdf(
    ...
    df_sorted = df_sector.sort_values(by="composite_quality_score", ascending=False).copy()
    ...
    # 3. Median KPI Cards
    story.append(Paragraph("<b>MEDIAN SECTOR PERFORMANCE METRICS</b>", section_heading_style))
    ...
    roe_color = PRO_GREEN.hexval() if roe_val > 15 else (CON_RED.hexval() if roe_val < 0 else NAVY_PRIMARY.hexval())
    roce_color = PRO_GREEN.hexval() if roce_val > 15 else (CON_RED.hexval() if roce_val < 0 else NAVY_PRIMARY.hexval())
    de_color = PRO_GREEN.hexval() if de_val < 0.5 else (CON_RED.hexval() if de_val > 1.5 else NAVY_PRIMARY.hexval())
    rev_color = PRO_GREEN.hexval() if rev_cagr_val > 10 else (CON_RED.hexval() if rev_cagr_val < 0 else NAVY_PRIMARY.hexval())
    pat_color = PRO_GREEN.hexval() if pat_cagr_val > 10 else (CON_RED.hexval() if pat_cagr_val < 0 else NAVY_PRIMARY.hexval())
    fcf_color = PRO_GREEN.hexval() if fcf_yield_val > 3.0 else (CON_RED.hexval() if fcf_yield_val < 0 else NAVY_PRIMARY.hexval())

    card_roe = make_sector_kpi_card("Median ROE", f"{roe_val:.1f}%", "Return on Equity", roe_color)
    card_roce = make_sector_kpi_card("Median ROCE", f"{roce_val:.1f}%", "Return on Capital", roce_color)
    card_de = make_sector_kpi_card("Median Debt/Equity", f"{de_val:.2f}x", "Leverage Ratio", de_color)
    card_rev = make_sector_kpi_card("Median Rev CAGR (5Y)", f"{rev_cagr_val:.1f}%", "Revenue Growth", rev_color)
    card_pat = make_sector_kpi_card("Median PAT CAGR (5Y)", f"{pat_cagr_val:.1f}%", "Net Profit Growth", pat_color)
    card_fcf = make_sector_kpi_card("Median FCF Yield", f"{fcf_yield_val:.2f}%", "Free Cash Flow / Mcap", fcf_color)

    kpis_data = [
        [card_roe, card_roce, card_de],
        [card_rev, card_pat, card_fcf]
    ]
    ...
    # Detailed notes
    notes_style = ParagraphStyle('Notes', parent=table_cell_style, fontSize=8, leading=10, textColor=TEXT_LIGHT)
    story.append(Paragraph("<i>Note: Medians are calculated using eligible constituents within this sector cohort. Financial metrics are winsorized to prevent outlier distortion.</i>", notes_style))
    ...
    story.append(Paragraph("<b>SECTOR CONSTITUENTS BREAKDOWN</b>", section_heading_style))
    ...
    for idx, row in df_sorted.iterrows():
        ...
        table_data.append([
            comp_cell,
            Paragraph(f"{roe_val:.1f}%" if pd.notnull(roe_val) else "N/A", table_cell_style),
            Paragraph(f"{roce_val:.1f}%" if pd.notnull(roce_val) else "N/A", table_cell_style),
            Paragraph(f"{rev_cagr:.1f}%" if pd.notnull(rev_cagr) else "N/A", table_cell_style),
            Paragraph(f"{pat_cagr:.1f}%" if pd.notnull(pat_cagr) else "N/A", table_cell_style),
            Paragraph(f"{de_val:.2f}x" if pd.notnull(de_val) else "N/A", table_cell_style),
            Paragraph(f"\u20b9{fcf_val:,.0f}" if pd.notnull(fcf_val) else "N/A", table_cell_style),
            Paragraph(f"<b>{score_val:.1f}</b>" if pd.notnull(score_val) else "N/A", table_cell_style),
            Paragraph(f"\u20b9{mcap_val:,.0f}" if pd.notnull(mcap_val) else "N/A", table_cell_style),
        ])
...
def generate_all_sector_reports(db_path: Optional[Path] = None, out_dir: Optional[Path] = None) -> None:
    ...
    try:
        ...
        df_latest = pd.merge(df_latest, df_mcap[["company_id", "market_cap_crore"]], on="company_id", how="left")
    except Exception as e:
    ...
    df_eligible["fcf_yield"] = df_eligible.apply(
        lambda r: (r["free_cash_flow_cr"] / r["market_cap_crore"] * 100.0)
        if pd.notnull(r["free_cash_flow_cr"]) and pd.notnull(r["market_cap_crore"]) and r["market_cap_crore"] > 0
        else 0.0,
        axis=1
    )
    ...
    logger.info(f"Identified {len(unique_sectors)} sectors for reporting: {unique_sectors}")
    ...
    for sector in unique_sectors:
        ...
        logger.info(f"Generating Sector Report: {pdf_name} with {len(df_sec)} constituents...")
        try:
            ...
            logger.error(f"Failed to generate report for sector {sector}: {e}", exc_info=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=str,
        help="Custom output directory for sector reports."
    )
