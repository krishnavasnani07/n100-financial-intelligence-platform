...
from src.reports.report_styles import (
    A4_WIDTH, A4_HEIGHT, MARGIN_POINTS, PRINTABLE_WIDTH, section_heading_style
)
from src.reports.report_builder import (
    NumberedCanvas, build_header, build_kpi_table, build_trend_table,
    build_pros_cons_table, build_allocation_valuation_table
)
...
def load_all_data(db_path: Path) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    sectors_df["company_id"] = sectors_df["company_id"].astype(str).str.strip().str.upper()
    ...
    ratios_df = ratios_df.dropna(subset=["year_int"]).sort_values(by=["company_id", "year_int"]).copy()

    # Load output data
    logger.info("Loading intelligence files (pros/cons, valuation, cashflow intelligence)...")
    pros_cons_file = OUTPUT_DIR / "pros_cons_generated.csv"
    if pros_cons_file.exists():
        pros_cons_df = pd.read_csv(pros_cons_file)
        pros_cons_df["company_id"] = pros_cons_df["company_id"].astype(str).str.strip().str.upper()
    else:
        logger.warning(f"pros_cons_generated.csv not found at {pros_cons_file}. Using empty default.")
        pros_cons_df = pd.DataFrame(columns=["company_id", "type", "text", "confidence_pct"])
    ...
    if val_summary_file.exists():
        ...
        logger.warning(f"valuation_summary.xlsx not found at {val_summary_file}. Using empty default.")
        val_df = pd.DataFrame(columns=["Company ID", "P/E Ratio", "FCF Yield %", "P/E vs. Sector Median %", "Valuation Flag"])
    ...
    if cashflow_intel_file.exists():
        ...
        logger.warning(f"cashflow_intelligence.xlsx not found at {cashflow_intel_file}. Using empty default.")
...
def generate_portfolio_summary_report(db_path: Path) -> Path:
    ...
    # Load data
    companies_df, sectors_df, ratios_df, pros_cons_df, val_df, cf_df = load_all_data(db_path)

    # Validate data
    warnings = validate_portfolio_cohort(companies_df, sectors_df, ratios_df, pros_cons_df, val_df, cf_df)
    if warnings:
    ...
    for _, row in val_df.iterrows():
        ...
        val_map[row["Company ID"]] = {
            ...
            "sec_median_pe": row.get("5Y Median P/E") or row.get("P/E Ratio"),  # Fallback if sector median pe is not named
    ...
    for idx, company in enumerate(cohort):
        ...
        # Get sector mapping
        broad_sec, sub_sec = sector_map.get(company_id, ("Unclassified", "Unclassified"))
        mapped_sector_name = map_sector(broad_sec, sub_sec)
        
        logger.info(f"[{idx+1}/92] Generating page for {company_id} ({company_name})...")
        ...
        # Get pros & cons
        co_pros = pros_cons_df[(pros_cons_df["company_id"] == company_id) & (pros_cons_df["type"] == "PRO")]
        co_cons = pros_cons_df[(pros_cons_df["company_id"] == company_id) & (pros_cons_df["type"] == "CON")]
        
        pros_list = co_pros.sort_values(by="confidence_pct", ascending=False)["text"].tolist()
        cons_list = co_cons.sort_values(by="confidence_pct", ascending=False)["text"].tolist()
        
        # Get Capital Allocation
        alloc_label = cf_map.get(company_id, "Mixed")
        
        # Get Valuation details
        val_info = val_map.get(company_id, {
            "pe": None,
            "fcf_yield": None,
            "sec_median_pe": None,
            "flag": "Fair"
        ...
        # 1. Header Table
        header_table = build_header(company_name, company_id, mapped_sector_name, sub_sec or broad_sec, latest_year)
        ...
        # 3. Trend Section
        story.append(Paragraph("<b>YEAR-OVER-YEAR TREND INDICATORS</b>", section_heading_style))
        trend_table = build_trend_table(latest_ratios, prev_ratios)
        story.append(trend_table)
        story.append(Spacer(1, 12))
        
        # 4. Pros & Cons Section
        story.append(Paragraph("<b>FINANCIAL INTELLIGENCE: PROS & CONS SENTIMENT</b>", section_heading_style))
        pc_table = build_pros_cons_table(pros_list, cons_list)
        story.append(pc_table)
        story.append(Spacer(1, 12))
        
        # 5. Capital Allocation & Valuation Section
        av_table = build_allocation_valuation_table(alloc_label, fcf_yield, pe_ratio, sector_median_pe, val_flag)
