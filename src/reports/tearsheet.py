...
# Set up logging
def setup_tearsheet_logger() -> logging.Logger:
    log_dir = BASE_DIR / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = log_dir / "tearsheet.log"
    ...
    # Add console stream handler for testing
...
def extract_year_int(yr_val: str) -> int:
    """Helper to extract year from date string (e.g. 'Mar 2024' -> 2024)."""
    if not yr_val or pd.isna(yr_val):
        return 0
...
def load_ratios(company_id: str, db_path: Optional[Path] = None) -> pd.DataFrame:
    ...
    try:
        ...
        if len(df) < 3:
            logger.warning(f"Company {company_id} has only {len(df)} years of data (< 3 required).")
        return df
    finally:
        conn.close()
...
def load_pros_cons(company_id: str, csv_path: Optional[Path] = None) -> Tuple[list[str], list[str]]:
    ...
    try:
        ...
        df_comp = df[df["company_id"].astype(str).str.strip().str.upper() == str(company_id).strip().upper()].copy()
        
        pros = df_comp[df_comp["type"] == "PRO"].sort_values(by="confidence_pct", ascending=False)["text"].tolist()
        cons = df_comp[df_comp["type"] == "CON"].sort_values(by="confidence_pct", ascending=False)["text"].tolist()
...
def build_navy_header(company: dict, latest_year: str) -> Table:
    ...
    sector = str(company.get("sub_sector") or company.get("broad_sector") or "Unclassified")
...
def build_kpi_table(ratios_df: pd.DataFrame) -> Table:
    ...
    card_roce = make_kpi_card("Return on Capital (ROCE)", roce_str, "Percentage", roce_color)
    ...
    if pd.isnull(npm_val):
        ...
        npm_color = PRO_GREEN.hexval() if npm_val > 12 else (CON_RED.hexval() if npm_val < 0 else NAVY_PRIMARY.hexval())
    card_npm = make_kpi_card("Net Profit Margin (NPM)", npm_str, "Percentage", npm_color)
    ...
    if pd.isnull(cagr_val):
        ...
        cagr_color = PRO_GREEN.hexval() if cagr_val > 10 else (CON_RED.hexval() if cagr_val < 0 else NAVY_PRIMARY.hexval())
    ...
    if pd.isnull(fcf_val):
        ...
        fcf_color = PRO_GREEN.hexval() if fcf_val > 0 else (CON_RED.hexval() if fcf_val < 0 else NAVY_PRIMARY.hexval())
    card_fcf = make_kpi_card("Free Cash Flow (FCF)", fcf_str, "INR in Crores", fcf_color)

    # Arrange in a 2x3 Grid Table
    data = [
        [card_roe, card_roce, card_npm],
        [card_de, card_cagr, card_fcf]
    ]
    ...
    kpi_table.setStyle(TableStyle([
        ...
        ('GRID', (0,0), (-1,-1), 1, colors.white), # white grid lines between cards
...
def generate_tearsheet(company_id: str, db_path: Optional[Path] = None) -> Path:
    ...
    try:
        ...
        logger.error(f"[{company_id}] Data loading or validation failed: {e}", exc_info=True)
        raise
    ...
    rev_chart, prof_chart = generate_revenue_net_profit_charts(pl_df, company_id, charts_dir)
    roe_roce_chart = generate_roe_roce_chart(ratios_df, company_id, charts_dir)
    bs_composition_chart = generate_balancesheet_composition_chart(bs_df, company_id, charts_dir)
    
    latest_cf_row = cf_df.iloc[-1]
    waterfall_chart = generate_cashflow_waterfall_chart(latest_cf_row, company_id, charts_dir)
    ...
    story.append(Paragraph("<b>REVENUE & PROFITABILITY TRENDS</b>", section_heading_style))
    ...
    story.append(Paragraph("<b>OPERATIONAL RETURN METRICS (10Y)</b>", section_heading_style))
    ...
    story.append(Paragraph("<b>BALANCE SHEET COMPOSITION & CASH FLOW ANALYSIS</b>", section_heading_style))
    ...
    heading_p = Paragraph("<b>FINANCIAL INSIGHTS & NLP SENTIMENT ANALYSIS</b>", section_heading_style)
    
    # Create header table for section 7
    sec7_header_table = Table([[heading_p, allocation_badge]], colWidths=[353, 170])
    sec7_header_table.setStyle(TableStyle([
        ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
        ('ALIGN', (1,0), (1,0), 'RIGHT'),
        ('LEFTPADDING', (0,0), (-1,-1), 0),
        ('RIGHTPADDING', (0,0), (-1,-1), 0),
    ...
    pros_flowables = [Paragraph("<b>Key Financial Strengths (Pros)</b>", ParagraphStyle('ProTitle', parent=section_heading_style, textColor=PRO_GREEN, fontSize=10, leading=12, spaceBefore=0, spaceAfter=4))]
    for p_text in pros_to_show:
        bullet_html = f"<font color='{PRO_GREEN.hexval()}'><b>&#9656;</b></font> {p_text}"
        pros_flowables.append(Paragraph(bullet_html, bullet_style))

    cons_flowables = [Paragraph("<b>Key Financial Concerns (Cons)</b>", ParagraphStyle('ConTitle', parent=section_heading_style, textColor=CON_RED, fontSize=10, leading=12, spaceBefore=0, spaceAfter=4))]
