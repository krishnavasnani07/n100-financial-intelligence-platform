...
def generate_revenue_net_profit_charts(
    df: pd.DataFrame, company_id: str, out_dir: Path
) -> tuple[Path, Path]:
    ...
    bars = ax.bar(
        years, revenue, color="#1B365D", width=0.6, edgecolor="#11223B", linewidth=0.5
    )
    ax.set_title(
        "Revenue (₹ Cr)", fontsize=9, fontweight="bold", color="#1B365D", pad=6
    )
    ...
    ax.set_title(
        "Net Profit (₹ Cr)", fontsize=9, fontweight="bold", color="#2E7D32", pad=6
    )
    ax.tick_params(axis="both", which="major", labelsize=7)
    ax.grid(axis="y", linestyle="--", alpha=0.5)
    sns.despine(ax=ax, left=True, bottom=False, right=True, top=True)
...
def generate_balancesheet_composition_chart(
    df: pd.DataFrame, company_id: str, out_dir: Path
) -> Path:
    ...
    equity = (
        df_last_10["reserves"].fillna(0) + df_last_10["equity_capital"].fillna(0)
    ).tolist()
...
def generate_cashflow_waterfall_chart(
    cf_row: pd.Series, company_id: str, out_dir: Path
) -> Path:
    ...
    ax.set_title(
        f"Cash Flow Breakdown: {cf_row.get('year', 'Latest')} (₹ Cr)",
        fontsize=9,
        fontweight="bold",
        color="#1B365D",
        pad=6,
    ...
    span = max_y - min_y
    ax.set_ylim(
        min_y - 0.15 * span if span else -10, max_y + 0.15 * span if span else 10
    )
