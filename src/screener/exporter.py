...
COLUMNS_DEFINITION = [
    ...
    {"field": "composite_quality_score", "header": "Composite Quality Score", "format": "0.00"},
    {"field": "return_on_equity_pct", "header": "ROE (%)", "format": "0.0"},
    {"field": "return_on_capital_employed_pct", "header": "ROCE (%)", "format": "0.0"},
    {"field": "net_profit_margin_pct", "header": "Net Profit Margin (%)", "format": "0.0"},
    {"field": "operating_profit_margin_pct", "header": "Operating Margin (%)", "format": "0.0"},
    ...
    {"field": "dividend_payout_ratio_pct", "header": "Dividend Payout (%)", "format": "0.0"},
...
def apply_conditional_coloring(ws: Any, df: pd.DataFrame, preset_name: str) -> None:
    ...
    for idx, row in df.iterrows():
        ...
        if preset_name == "Quality Compounder":
            ...
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["return_on_equity_pct"]), _get_val("return_on_equity_pct") > 15.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["debt_to_equity"]), sector == "financials" or _get_val("debt_to_equity") < 1.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["free_cash_flow_cr"]), _get_val("free_cash_flow_cr") > 0.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["revenue_cagr_5yr"]), _get_val("revenue_cagr_5yr") > 10.0)

        elif preset_name == "Value Pick":
            # P/E < 20, P/B < 3.0, D/E < 2.0, Dividend Yield > 1%
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["pe"]), _get_val("pe") < 20.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["pb"]), _get_val("pb") < 3.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["debt_to_equity"]), sector == "financials" or _get_val("debt_to_equity") < 2.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["dividend_yield"]), _get_val("dividend_yield") > 1.0)

        elif preset_name == "Growth Accelerator":
            # PAT CAGR 5yr > 20%, Revenue CAGR 5yr > 15%, D/E < 2.0
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["pat_cagr_5yr"]), _get_val("pat_cagr_5yr") > 20.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["revenue_cagr_5yr"]), _get_val("revenue_cagr_5yr") > 15.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["debt_to_equity"]), sector == "financials" or _get_val("debt_to_equity") < 2.0)

        elif preset_name == "Dividend Champion":
            # Dividend Yield > 2%, Dividend Payout < 80%, FCF > 0
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["dividend_yield"]), _get_val("dividend_yield") > 2.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["dividend_payout_ratio_pct"]), _get_val("dividend_payout_ratio_pct") < 80.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["free_cash_flow_cr"]), _get_val("free_cash_flow_cr") > 0.0)

        elif preset_name == "Debt-Free Blue Chip":
            # D/E = 0, ROE > 12%, Revenue > 5000 Crore
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["debt_to_equity"]), sector == "financials" or _get_val("debt_to_equity") == 0.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["return_on_equity_pct"]), _get_val("return_on_equity_pct") > 12.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["sales"]), _get_val("sales") > 5000.0)

        elif preset_name == "Turnaround Watch":
            # Revenue CAGR 3yr > 10%, FCF positive, D/E declining YoY
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["revenue_cagr_3yr"]), _get_val("revenue_cagr_3yr") > 10.0)
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["free_cash_flow_cr"]), _get_val("free_cash_flow_cr") > 0.0)
            
            de_dec = row.get("de_declining_yoy")
            is_declining = de_dec == True or de_dec == 1 or str(de_dec).strip().lower() in ["true", "1"]
            apply_threshold_cell_fill(ws.cell(row=r_idx, column=col_indices["de_declining_yoy"]), is_declining)
...
def format_data_sheet(ws: Any, col_definitions: List[Dict[str, Any]]) -> None:
    ...
    for row in range(2, ws.max_row + 1):
        ...
        row_fill = PatternFill(start_color="F9FBFD" if row % 2 == 0 else "FFFFFF", 
...
def populate_data_sheet(ws: Any, df: pd.DataFrame, col_definitions: List[Dict[str, Any]]) -> None:
...
def create_summary_sheet(wb: openpyxl.Workbook, rankings_df: pd.DataFrame, preset_results: Dict[str, pd.DataFrame]) -> None:
    ...
    # Title block
    ws.cell(row=1, column=1, value="NIFTY 100 FINANCIAL INTELLIGENCE PLATFORM").font = Font(name="Calibri", size=16, bold=True, color="1B365D")
    ws.cell(row=2, column=1, value="Screener results & Composite Quality Rankings Summary").font = Font(name="Calibri", size=11, italic=True, color="595959")
    ws.cell(row=3, column=1, value=f"Report Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}").font = Font(name="Calibri", size=9, color="7F7F7F")
    
    # 1. Universe Quality Statistics
    ws.cell(row=5, column=1, value="Universe Quality Summary Statistics").font = Font(name="Calibri", size=13, bold=True, color="1B365D")
    
    stats = [
        ("Total Companies Analyzed", len(rankings_df), "0"),
        ("Average Quality Score", rankings_df["composite_quality_score"].mean(), "0.00"),
    ...
    ws.cell(row=13, column=1, value="Screener Preset Statistics").font = Font(name="Calibri", size=13, bold=True, color="1B365D")
    
    preset_headers = ["Preset Screener", "Description", "Companies Passing", "Avg. Composite Score"]
...
def generate_reports() -> None:
    ...
    for name in presets:
        ...
        df_sorted = df_res.sort_values(by="composite_quality_score", ascending=False).reset_index(drop=True)
