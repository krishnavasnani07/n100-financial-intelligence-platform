...
def render_sector_bubble_chart(df: pd.DataFrame, sector_name: str):
    ...
    df_plot["bubble_size"] = df_plot["index_weight_pct"].apply(
        lambda x: max(float(x), 0.05)
    )

    # Format values for labels
    df_plot["Sales (Cr)"] = df_plot["sales"].apply(
        lambda x: f"₹{x:,.2f} Cr" if pd.notnull(x) else "N/A"
    )
    df_plot["ROE (%)"] = df_plot["return_on_equity_pct"].apply(
        lambda x: f"{x:.2f}%" if pd.notnull(x) else "N/A"
    )
    df_plot["Index Weight (%)"] = df_plot["index_weight_pct"].apply(
        lambda x: f"{x:.2f}%" if pd.notnull(x) else "N/A"
    )
