...
def classify_company_capital_allocation(row: pd.Series) -> str:
    ...
    if (pd.notnull(div_yield) and div_yield > 2.0) or (pd.notnull(payout) and payout > 30.0):
...
def render_capital_allocation_treemap(df: pd.DataFrame):
    ...
    df_plot['block_size'] = df_plot['index_weight_pct'].apply(lambda x: max(float(x), 0.05))

    # Add custom formatting for details
    df_plot['ROE (%)'] = df_plot['return_on_equity_pct'].apply(lambda x: f"{x:.1f}%" if pd.notnull(x) else "N/A")
    df_plot['D/E (x)'] = df_plot['debt_to_equity'].apply(lambda x: f"{x:.2f}x" if pd.notnull(x) else "N/A")
    df_plot['Composite Score'] = df_plot['composite_quality_score'].apply(lambda x: f"{x:.2f}" if pd.notnull(x) else "N/A")
