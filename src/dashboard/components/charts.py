...
def render_revenue_chart(df_pl: pd.DataFrame):
    ...
    fig.update_layout(
        ...
        yaxis=dict(showgrid=True, gridcolor='rgba(255, 255, 255, 0.1)', title="Revenue (INR Cr)")
    )
...
def render_profit_chart(df_pl: pd.DataFrame):
    ...
    fig.update_layout(
        ...
        font=dict(color='#ffffff'),
        xaxis=dict(showgrid=False, title="Financial Year"),
        yaxis=dict(showgrid=True, gridcolor='rgba(255, 255, 255, 0.1)', title="Net Profit (INR Cr)")
    )
