...
def calculate_normalized_radar_metrics(df_universe: pd.DataFrame) -> pd.DataFrame:
    ...
    for metric, lower_is_better in metrics_config.items():
        ...
        if p90 > p10:
            ...
            if lower_is_better:
                scores = 100.0 * (p90 - capped) / (p90 - p10) if 'capped' in locals() else 100.0 * (p90 - clipped) / (p90 - p10)
            else:
...
def render_peer_radar(selected_company_id: str, df_raw: pd.DataFrame, df_norm: pd.DataFrame):
    """
