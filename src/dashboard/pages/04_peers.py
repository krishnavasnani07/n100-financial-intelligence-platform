...
st.markdown(
    "<p style='color:#8892b0; margin-top:-15px;'>Compare a company's financial strength and quality metrics directly with its sector peers.</p>",
...
st.sidebar.markdown(
    "<h3 style='margin-bottom:5px; font-weight:700;'>👥 Peer Selector</h3>",
    unsafe_allow_html=True,
)
...
company_options = [
    ...
    for _, row in df_sector_companies.iterrows()
]
...
with col_radar:
    ...
    st.markdown(
        "*Shows winsorized 0-100 normalized scores (outer bounds indicate top decile performance)*"
    )
