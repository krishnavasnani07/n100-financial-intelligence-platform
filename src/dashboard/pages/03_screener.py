...
st.markdown("<h1 style='font-weight:800;'>🔍 Investment Screener</h1>", unsafe_allow_html=True)
st.markdown("<p style='color:#8892b0; margin-top:-15px;'>Filter Nifty 100 companies dynamically based on key financial metrics or use preset templates.</p>", unsafe_allow_html=True)
st.markdown("---")
...
if num_matches > 0:
    ...
    with col3:
        st.metric(
            label="Average PE",
            value=f"{avg_pe:.2f}x" if pd.notnull(avg_pe) else "N/A"
        )
else:
