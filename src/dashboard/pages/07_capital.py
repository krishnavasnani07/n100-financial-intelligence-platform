...
st.markdown(
    "<h1 style='font-weight:800;'>💰 Capital Allocation Intelligence</h1>",
    unsafe_allow_html=True,
)
st.markdown(
    "<p style='color:#8892b0; margin-top:-15px;'>Categorize Nifty 100 constituents based on their historical capital allocation efficiency, growth reinvestment, and payout strategies.</p>",
    unsafe_allow_html=True,
)
st.markdown("---")
...
with tab2:
    ...
    if summary_path.exists():
        ...
        st.info(
            "Allocation distribution summary not found. Please run the ETL pipeline first."
        )
    ...
    if changes_path.exists():
        ...
        if sel_cat != "All Changes":
            df_changes_filt = df_changes_filt[
                df_changes_filt["change_category"] == sel_cat
            ]
        if sel_yr != "All Years":
        ...
        st.markdown(
            f"**Found {len(df_changes_filt)} transition events matching criteria.**"
        )

        # Format table for presentation
        df_changes_table = df_changes_filt.copy()
        df_changes_table.rename(
            columns={
                "company_id": "Ticker",
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Transition Category": st.column_config.TextColumn(
                    "Transition Category"
                ),
            },
        )
