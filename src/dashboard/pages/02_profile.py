...
from src.dashboard.components.charts import render_revenue_chart, render_profit_chart, render_roe_roce_trend
...
with st.container():
    ...
    with col_desc:
        st.markdown(f"<h2 style='margin-bottom: 2px; font-weight: 700;'>{company_info['company_name']}</h2>", unsafe_allow_html=True)
        st.markdown(f"**NSE Ticker:** `{selected_ticker}` | **Sector:** {sector_name} | **Sub-Sector:** {sub_sector_name}")
        st.markdown(company_info.get('about_company') or "No description available for this company.")
...
with col1:
    ...
    kpi_card("ROE", f"{roe:.2f}%" if pd.notnull(roe) else "N/A", "Return on Equity", "positive" if (roe or 0) >= 20.0 else ("neutral" if (roe or 0) >= 12.0 else "negative"))
with col2:
    roce = latest_ratios.get('return_on_capital_employed_pct')
    kpi_card("ROCE", f"{roce:.2f}%" if pd.notnull(roce) else "N/A", "Return on Capital", "positive" if (roce or 0) >= 20.0 else ("neutral" if (roce or 0) >= 12.0 else "negative"))
with col3:
    npm = latest_ratios.get('net_profit_margin_pct')
    kpi_card("Net Profit Margin", f"{npm:.2f}%" if pd.notnull(npm) else "N/A", "Net Margin", "positive" if (npm or 0) >= 15.0 else ("neutral" if (npm or 0) >= 8.0 else "negative"))
with col4:
    rev_cagr = latest_ratios.get('revenue_cagr_5yr')
    kpi_card("Revenue CAGR (5y)", f"{rev_cagr:.2f}%" if pd.notnull(rev_cagr) else "N/A", "5-Year Revenue Growth", "positive" if (rev_cagr or 0) >= 12.0 else ("neutral" if (rev_cagr or 0) >= 6.0 else "negative"))
with col5:
    de = latest_ratios.get('debt_to_equity')
    kpi_card("Debt to Equity", f"{de:.2f}x" if pd.notnull(de) else "N/A", "Leverage Ratio", "positive" if (de or 99) < 0.5 else ("neutral" if (de or 0) < 1.5 else "negative"))
with col6:
    fcf = latest_ratios.get('free_cash_flow_cr')
    kpi_card("Latest FCF", f"₹{fcf:,.1f} Cr" if pd.notnull(fcf) else "N/A", "Free Cash Flow", "positive" if (fcf or 0) > 0 else "negative")
...
if len(df_pl_sorted) >= 2:
    ...
    if pd.notnull(latest_rev) and pd.notnull(prev_rev) and prev_rev > 0:
        if latest_rev > prev_rev:
            ...
            pros.append(f"YoY Revenue expansion of {growth:.2f}% (latest vs previous period)")
        else:
            decline = ((prev_rev - latest_rev) / prev_rev) * 100
            cons.append(f"YoY Revenue contraction of {decline:.2f}% (latest vs previous period)")
