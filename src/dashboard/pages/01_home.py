...
available_years = sorted(
    [int(y) for y in df_master["year_int"].dropna().unique() if 2019 <= y <= 2024]
)
if not available_years:
    available_years = [2019, 2020, 2021, 2022, 2023, 2024]

selected_year = st.sidebar.selectbox(
    "Select Filter Year", available_years, index=len(available_years) - 1
)
...
with col1:
    kpi_card(
        "Average ROE",
        f"{avg_roe:.2f}%" if pd.notnull(avg_roe) else "N/A",
        "Avg Return on Equity",
        "positive" if (avg_roe or 0) >= 15 else "neutral",
    )
with col2:
    kpi_card(
        "Median PE",
        f"{median_pe:.2f}x" if pd.notnull(median_pe) else "N/A",
        "Median Price/Earnings",
        "positive" if (median_pe or 99) < 25 else "neutral",
    )
with col3:
    kpi_card(
        "Median D/E",
        f"{median_de:.2f}x" if pd.notnull(median_de) else "N/A",
        "Median Debt to Equity",
        "positive" if (median_de or 99) < 0.5 else "neutral",
    )
with col4:
...
with col5:
    kpi_card(
        ...
        "5-Year Revenue CAGR",
        "positive" if (median_rev_cagr or 0) >= 10 else "neutral",
    )
with col6:
    kpi_card(
        "Debt-Free Cos",
        ...
        "positive" if debt_free_count > 0 else "neutral",
    )
...
with col_table:
    ...
    if not df_filtered.empty:
        df_top5 = (
            df_filtered.sort_values(by="composite_quality_score", ascending=False)
            .head(5)
            .copy()
        )
