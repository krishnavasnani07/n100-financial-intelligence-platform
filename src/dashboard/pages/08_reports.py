...
st.markdown("<h1 style='font-weight:800;'>📄 Annual Reports Browser</h1>", unsafe_allow_html=True)
st.markdown("<p style='color:#8892b0; margin-top:-15px;'>Search and view PDF annual reports and BSE corporate filings for Nifty 100 constituents.</p>", unsafe_allow_html=True)
st.markdown("---")
...
def get_cached_report_data():
    try:
        ...
        df = pd.merge(df_docs, df_companies[['id', 'company_name']], left_on='company_id', right_on='id', how='left')
        return df
...
# Sidebar Filters
st.sidebar.markdown("<h3 style='margin-bottom:5px; font-weight:700;'>🔎 Report Filters</h3>", unsafe_allow_html=True)

# 1. Company Filter
company_list = sorted(list(df_docs_all[['company_id', 'company_name']].dropna().drop_duplicates().apply(
    lambda r: f"{r['company_id']} - {r['company_name']}", axis=1
).tolist()))
...
selected_years = st.sidebar.multiselect("Filter by Financial Year:", options=years_list, default=[])
