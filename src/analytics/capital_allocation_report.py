...
def categorize_transition(prev: str, curr: str) -> str:
    ...
    # Recovery / Improving
    if prev in ["Distress Signal", "Growth Funded by Debt"] and curr in [
        "Cash Accumulator",
        "Reinvestor",
        "Shareholder Returns",
    ]:
    ...
    if prev in ["Reinvestor", "Shareholder Returns", "Cash Accumulator"] and curr in [
    ...
    # Improving
    if prev in ["Mixed", "Pre-Revenue", "Liquidating Assets"] and curr in [
        "Reinvestor",
        "Cash Accumulator",
    ]:
        return "Improving"
...
def verify_capital_allocation(df_alloc: pd.DataFrame, db_path: Path) -> List[str]:
    ...
    if not duplicates.empty:
        errors.append(
            f"Duplicate (company_id, year) records found: {duplicates[['company_id', 'year']].values.tolist()}"
        )
    ...
    if not invalid_rows.empty:
        errors.append(
            f"Invalid pattern labels: {invalid_rows[['company_id', 'year', 'pattern_label']].values.tolist()}"
        )
    ...
    if missing:
        errors.append(
            f"Missing {len(missing)} company-year records in allocation dataset."
        )
...
def run_capital_allocation_report(
    ...
    pattern_counts["percentage"] = (
        pattern_counts["company_count"] / len(df_latest) * 100
    ).round(2)
    ...
    logger.info(
        f"Exported pattern strategy transitions to {changes_path}. Changes count: {change_count}"
    )
    ...
    if excel_path.exists():
        ...
        try:
            ...
            alloc_latest_map = dict(
                zip(df_latest["company_id"], df_latest["pattern_label"])
            )

            df_intel["company_id_upper"] = (
                df_intel["company_id"].astype(str).str.strip().str.upper()
            )
            df_intel["capital_allocation_label"] = (
                df_intel["company_id_upper"].map(alloc_latest_map).fillna("Mixed")
            )
            ...
            logger.info(
                f"Integrated and updated cashflow_intelligence.xlsx at {excel_path}"
            )
        except Exception as e:
            logger.error(
                f"Failed to update cashflow_intelligence.xlsx: {e}", exc_info=True
            )
    ...
    print(
        f"    Validation Status          : {'PASSED' if not errors else 'WARNINGS LOGGED'}"
    )
    print("-" * 80)

    return {"summary": df_latest, "changes": df_changes, "change_count": change_count}
