...
from src.config.settings import BASE_DIR, DB_PATH, RAW_DATA_DIR, OUTPUT_DIR
...
def setup_intelligence_logging() -> logging.Logger:
    log_dir = BASE_DIR / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = log_dir / "cashflow_intelligence.log"
    
    ci_logger = logging.getLogger("cashflow_intelligence")
    for h in list(ci_logger.handlers):
        ci_logger.removeHandler(h)
        
    fh = logging.FileHandler(log_file, mode="a", encoding="utf-8")
    formatter = logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
    fh.setFormatter(formatter)
    ci_logger.addHandler(fh)
    ci_logger.setLevel(logging.INFO)
    return ci_logger


logger = setup_intelligence_logging()
...
def load_cashflow(db_path: Path) -> pd.DataFrame:
    ...
    try:
        df = pd.read_sql_query("SELECT company_id, year, operating_activity, investing_activity, financing_activity FROM cashflow", conn)
...
def load_profit_loss(db_path: Path) -> pd.DataFrame:
    conn = sqlite3.connect(str(db_path))
    try:
        df = pd.read_sql_query("SELECT company_id, year, sales, expenses, operating_profit, net_profit FROM profitandloss", conn)
        df["company_id"] = df["company_id"].astype(str).str.strip().str.upper()
        return df
    finally:
        conn.close()
...
def load_balance_sheet(db_path: Path) -> pd.DataFrame:
    conn = sqlite3.connect(str(db_path))
    try:
        df = pd.read_sql_query("SELECT company_id, year, borrowings, total_assets FROM balancesheet", conn)
        df["company_id"] = df["company_id"].astype(str).str.strip().str.upper()
        return df
    finally:
        conn.close()
...
def load_ratios(db_path: Path) -> pd.DataFrame:
    conn = sqlite3.connect(str(db_path))
    try:
        df = pd.read_sql_query("SELECT company_id, year, debt_to_equity, return_on_equity_pct, free_cash_flow_cr FROM financial_ratios", conn)
        df["company_id"] = df["company_id"].astype(str).str.strip().str.upper()
        return df
    finally:
        conn.close()
...
def validate_inputs(
    ...
    if df_cf[["operating_activity", "investing_activity", "financing_activity"]].isnull().any().any():
...
def run_cashflow_intelligence_pipeline(
    ...
    try:
        ...
        # Validation checks
        warnings = validate_inputs(df_cf, df_pl, df_bs, df_ratios, df_companies, df_alloc)
        for w in warnings:
            logger.warning(w)
        ...
        conn.close()
        df_sectors["company_id"] = df_sectors["company_id"].astype(str).str.strip().str.upper()
    ...
    if not df_alloc.empty:
        ...
        df_alloc_latest = df_alloc.sort_values(by="year_int").drop_duplicates(subset=["company_id"], keep="last")
        alloc_map = dict(zip(df_alloc_latest["company_id"], df_alloc_latest["pattern_label"]))
    else:
        alloc_map = {}
    ...
    for idx, comp_row in df_companies.iterrows():
        ...
        if not history:
            logger.warning(f"No history found for company: {company_id}. Generating default entry.")
        ...
        deleveraging = bool(
            cff is not None and cff < 0 and
            borrowings_latest is not None and borrowings_prev is not None and
            borrowings_latest < borrowings_prev
        )
    ...
    if df_alerts.empty:
        ...
        df_alerts = pd.DataFrame(columns=["company_id", "company_name", "CFO", "CFF", "Net Profit", "Distress Reason"])
    ...
    if len(df_intel) != 92:
        val_msg.append(f"Validation warning: Processed {len(df_intel)} companies instead of 92.")
...
def export_excel_intelligence(df: pd.DataFrame, output_path: Path):
    ...
    try:
        ...
        writer.close()
        logger.info(f"Successfully exported styled cashflow intelligence workbook to {output_path}")
        
    except Exception as e:
        logger.error(f"Failed to style cashflow intelligence Excel report: {e}", exc_info=True)
