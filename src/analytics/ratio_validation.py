...
def categorize_ratio_mismatch(
    ...
    if source == 0:
        return (
            "DATA_SOURCE_ISSUE",
            "Source value is zero, which is not plausible for this ratio.",
        )

    if abs(float(source)) < 1 and abs(float(computed)) > 10:
        return (
            "DATA_SOURCE_ISSUE",
            "Source value appears malformed for a percentage-based ratio.",
        )

    if diff <= 5.0:
        return (
            "VERSION_DIFFERENCE",
            "Difference is small and consistent with a minor version/source change.",
        )

    return (
        "FORMULA_DISCREPANCY",
        "Difference is too large to be a simple version mismatch.",
    )
...
def build_ratio_mismatch_entries(
    ...
    merged = pd.merge(
        computed_df[[company_id_column, computed_column]].rename(
            columns={computed_column: "computed"}
        ),
        source_df[[company_id_column, source_column]].rename(
    ...
    for row in merged.itertuples(index=False):
        ...
        if pd.isna(computed) or pd.isna(source):
            category, reason = (
                "DATA_SOURCE_ISSUE",
                "Missing values prevent a meaningful comparison.",
            )
        else:
            category, reason = categorize_ratio_mismatch(
                ratio_name, float(computed), float(source)
            )

        diff = (
            abs(float(computed) - float(source))
            if not pd.isna(computed) and not pd.isna(source)
            else None
        )
        entries.append(
            {
