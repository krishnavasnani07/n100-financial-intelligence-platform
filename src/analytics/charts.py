...
def generate_profitability_charts(csv_log_path: Path, output_dir: Path) -> Path:
    ...
    fig.suptitle("Nifty 100 Financial Profitability Ratio Distributions", fontsize=18, fontweight="bold", y=0.98)
    ...
    for i, ratio in enumerate(ratios):
        ...
        sns.histplot(trimmed, kde=True, ax=ax, color=colors[i], bins=30, edgecolor="black", alpha=0.6)
        ...
        ax.set_title(f"{ratio} Distribution (96% Central Trimmed)", fontsize=13, fontweight="bold")
