...
def get_market_cap_history(ticker: str):
    ...
    try:
        ...
        if not path.exists():
            raise HTTPException(status_code=500, detail="market_cap.xlsx raw data file not found.")
        ...
        if df_company.empty:
            raise HTTPException(status_code=404, detail=f"No valuation history found for company '{ticker}'.")
        ...
        for _, row in df_company.iterrows():
            history.append(clean_dict_nans({
        ...
        raise HTTPException(status_code=500, detail=f"Valuation history query error: {e}")
