...
def generate_company_summary(company_id: str, row: Dict[str, any]) -> str:
    ...
    if roe and roe > 20:
        ...
        profit_str = (
            f"recorded moderate profitability with an ROE of {roe}% and ROCE of {roce}%"
        )
    ...
    if de is not None:
        if de == 0:
            ...
            leverage_str = (
                f"very conservative leverage structure (Debt-to-Equity of {de})"
            )
        elif de < 1.5:
    ...
    margin_str = (
        f"operating margin of {margin}%" if margin else "stable operating margins"
    )
    growth_pieces = []
...
def get_company_insights_data(
    ...
    df_latest = df.sort_values(by="year_int", ascending=False).drop_duplicates(
...
def generate_pdf_report(company_id: str, insights: Dict[str, any]) -> BytesIO:
    ...
    table_data = [
        ...
        [
            Paragraph("Sector", td_style),
            Paragraph(str(ratios.get("Sector", "N/A")), td_style),
        ],
        [
            Paragraph("Return on Equity (ROE %)", td_style),
            Paragraph(f"{ratios.get('ROE', 'N/A')}%", td_style),
        ],
        [
            Paragraph("Return on Capital Employed (ROCE %)", td_style),
            Paragraph(f"{ratios.get('ROCE', 'N/A')}%", td_style),
        ],
        [
            Paragraph("Operating Margin (%)", td_style),
            Paragraph(f"{ratios.get('Operating Margin', 'N/A')}%", td_style),
        ],
        [
            Paragraph("Revenue CAGR (5-Year %)", td_style),
            Paragraph(f"{ratios.get('Revenue CAGR', 'N/A')}%", td_style),
        ],
        [
            Paragraph("PAT CAGR (5-Year %)", td_style),
            Paragraph(f"{ratios.get('PAT CAGR', 'N/A')}%", td_style),
        ],
        [
            Paragraph("Debt to Equity Ratio", td_style),
            Paragraph(str(ratios.get("Debt to Equity", "N/A")), td_style),
        ],
        [
            Paragraph("Interest Coverage Ratio", td_style),
            Paragraph(str(ratios.get("Interest Coverage", "N/A")), td_style),
        ],
        [
            Paragraph("Composite Quality Score", td_style),
            Paragraph(
                f"{ratios.get('Composite Quality Score', 'N/A')} / 100", td_style
            ),
        ],
    ]
    ...
    story.append(
        Paragraph(
            "Disclaimer: This report is dynamically generated for informational and screening purposes only and does not constitute financial, investment, or legal advice.",
            disclaimer_style,
        )
    )
