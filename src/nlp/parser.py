...
def load_analysis_data(filepath: Optional[Path] = None) -> pd.DataFrame:
    ...
    required_cols = ["company_id", "compounded_sales_growth", "compounded_profit_growth", "stock_price_cagr", "roe"]
...
def load_companies(filepath: Optional[Path] = None) -> pd.DataFrame:
    ...
    if len(dup_ids) > 0:
        logger.warning(f"Duplicate company IDs detected in companies sheet: {list(dup_ids)}")
...
def load_growth_engine_output(filepath: Optional[Path] = None) -> Optional[pd.DataFrame]:
    """Loads precomputed growth engine summaries from output/growth_summary.csv."""
    file_path = Path(filepath) if filepath else settings.BASE_DIR / "output" / "growth_summary.csv"
    if not file_path.exists():
        logger.warning(f"Growth summary file not found at {file_path.resolve()}. Skipping load.")
        return None
...
def run_nlp_pipeline():
    ...
    try:
        ...
        valid_tickers = set(df_companies["id"].dropna().astype(str).str.strip().unique())
    ...
    for idx, row in df_analysis.iterrows():
        ...
        if company_id not in valid_tickers:
            parser_log.warning(f"Company ID '{company_id}' not found in companies list. Skipping.")
            continue
        ...
        for col_name, metric_type in METRIC_MAPPING.items():
            ...
            if pd.isna(cell_text) or not str(cell_text).strip():
                ...
                parser_log.warning(f"[{company_id}] {metric_type} parsing failed: Empty text.")
                continue
            ...
            if not parsed_matches:
                ...
                failures.append({
                    ...
                    "failure_reason": "Invalid format" if "Year" in str(cell_text) or "%" in str(cell_text) else "Unexpected wording"
                })
                failed_parses_count += 1
                parser_log.warning(f"[{company_id}] {metric_type} parsing failed: Invalid format/wording for '{cell_text}'.")
            else:
                # Parsed successfully
                for match in parsed_matches:
                    ...
                    parser_log.debug(f"[{company_id}] {metric_type} parsed: {match['period_years']}Y -> {match['value_pct']}%")
    ...
    if not df_parsed.empty:
        ...
        df_parsed = df_parsed.sort_values(by=["company_id", "metric_type", "period_years"])
        parsed_out_path = output_dir / "analysis_parsed.csv"
        df_parsed.to_csv(parsed_out_path, index=False)
        parser_log.info(f"Structured dataset written to {parsed_out_path.resolve()} with {len(df_parsed)} rows.")
    else:
        parser_log.warning("No successfully parsed records. analysis_parsed.csv not written.")
    ...
    parser_log.info(f"Parse failures report written to {failures_out_path.resolve()} with {len(df_failures)} rows.")
    ...
    if not df_parsed.empty:
        for idx, row in df_parsed.iterrows():
            ...
            computed_val = get_computed_value(company_id, metric_type, period_years, db_conn, df_growth)
            ...
            if computed_val is None:
                ...
                parser_log.info(f"[{company_id}] {metric_type} {period_years}Y: No computed value found for comparison. Flagged for review.")
            else:
            ...
            validation_records.append({
                ...
                "computed_value": computed_val if computed_val is not None else np.nan,
    ...
    parser_log.info(f"CAGR validation report written to {val_out_path.resolve()} with {len(df_val)} rows.")
