...
class FinancialRule:
    ...
    def evaluate(self, history: List[Dict[str, Any]], sector: str) -> Optional[Dict[str, Any]]:
        """
...
def register_rule(rule_id: str, rule_type: str, message: str):
    ...
    def decorator(funcs: tuple[Callable, Callable]):
        ...
        RULES_REGISTRY.append(FinancialRule(rule_id, rule_type, cond_fn, message, conf_fn))
        return funcs
    return decorator
...
def pro_03_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and history[-1].get("debt_to_equity") is not None and history[-1]["debt_to_equity"] == 0.0
...
def pro_04_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and history[-1].get("revenue_cagr_5yr") is not None and history[-1]["revenue_cagr_5yr"] > 15.0
...
def pro_05_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and history[-1].get("operating_profit_margin_pct") is not None and history[-1]["operating_profit_margin_pct"] > 20.0
...
def pro_06_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and history[-1].get("pat_cagr_5yr") is not None and history[-1]["pat_cagr_5yr"] > 15.0
...
register_rule(
    "PRO-06", "PRO",
    "Strong profit growth indicates robust bottom-line performance."
...
def pro_07_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and sector != "Financials" and history[-1].get("interest_coverage") is not None and history[-1]["interest_coverage"] > 5.0
...
def pro_08_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and history[-1].get("dividend_yield_pct") is not None and history[-1]["dividend_yield_pct"] > 2.0
...
def pro_09_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and history[-1].get("eps_cagr_5yr") is not None and history[-1]["eps_cagr_5yr"] > 15.0
...
def pro_10_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 3 and (
        history[-1].get("return_on_equity_pct") is not None and
        history[-2].get("return_on_equity_pct") is not None and
        history[-3].get("return_on_equity_pct") is not None and
        history[-1]["return_on_equity_pct"] > history[-2]["return_on_equity_pct"] > history[-3]["return_on_equity_pct"]
    )
...
def pro_12_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 3 and (
        history[-1].get("total_assets", 0) > history[-2].get("total_assets", 0) > history[-3].get("total_assets", 0) and
        history[-1].get("borrowings", 999) < history[-2].get("borrowings", 999) < history[-3].get("borrowings", 999)
    )
...
def con_01_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and sector != "Financials" and history[-1].get("debt_to_equity") is not None and history[-1]["debt_to_equity"] > 2.0
...
def con_03_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 3 and (
        history[-1].get("operating_profit_margin_pct") is not None and
        history[-2].get("operating_profit_margin_pct") is not None and
        history[-3].get("operating_profit_margin_pct") is not None and
        history[-1]["operating_profit_margin_pct"] < history[-2]["operating_profit_margin_pct"] < history[-3]["operating_profit_margin_pct"]
    )

def con_03_conf(history: List[Dict[str, Any]], sector: str) -> float:
    diff = history[-3]["operating_profit_margin_pct"] - history[-1]["operating_profit_margin_pct"]
    return 65.0 + diff * 2.0
...
def con_04_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and history[-1].get("net_profit") is not None and history[-1]["net_profit"] < 0.0
...
register_rule(
    "CON-04", "CON",
    "Company reported a net loss in the latest period."
...
def con_05_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 2 and history[-1].get("sales") is not None and history[-2].get("sales") is not None and history[-1]["sales"] < history[-2]["sales"]
...
def con_06_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and sector != "Financials" and history[-1].get("interest_coverage") is not None and history[-1]["interest_coverage"] < 1.5
...
def con_07_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and history[-1].get("dividend_payout_ratio_pct") is not None and 100.0 < history[-1]["dividend_payout_ratio_pct"] < 1000.0
...
def con_08_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 3 and (
        history[-1].get("total_debt_cr", 0) > history[-2].get("total_debt_cr", 0) > history[-3].get("total_debt_cr", 0)
    )

def con_08_conf(history: List[Dict[str, Any]], sector: str) -> float:
    diff = (history[-1]["total_debt_cr"] - history[-3]["total_debt_cr"]) / max(history[-3]["total_debt_cr"], 1.0)
    return 65.0 + diff * 100.0

register_rule(
    "CON-08", "CON",
    "Steadily rising borrowings increases leverage and financial risk."
...
def con_09_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 3 and (
        history[-1].get("earnings_per_share") is not None and
        history[-2].get("earnings_per_share") is not None and
        history[-3].get("earnings_per_share") is not None and
        history[-1]["earnings_per_share"] < history[-2]["earnings_per_share"] < history[-3]["earnings_per_share"]
    )
...
def con_10_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and history[-1].get("return_on_capital_employed_pct") is not None and history[-1]["return_on_capital_employed_pct"] < 10.0
...
def con_12_cond(history: List[Dict[str, Any]], sector: str) -> bool:
    return len(history) >= 1 and history[-1].get("revenue_cagr_5yr") is not None and history[-1]["revenue_cagr_5yr"] < 5.0
