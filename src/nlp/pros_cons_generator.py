...
def load_generator_data(
    ...
    if cf_summary_file.exists():
        ...
        logger.warning(f"cashflow_summary.csv not found at {cf_summary_file.resolve()}. Using empty template.")
    ...
    if analysis_parsed_file.exists():
        ...
        df_analysis = pd.DataFrame(columns=["company_id", "metric_type", "period_years", "value_pct"])
    ...
    if mcap_file.exists():
        ...
        logger.warning(f"market_cap.xlsx not found at {mcap_file}. Using empty template.")
    ...
    return df_companies, df_sectors, df_financials, df_cf, df_div_yield(df_mcap), df_analysis
...
def compile_company_histories(
    ...
    if not df_cflow.empty and "company_id" in df_cflow.columns:
        df_cflow["company_id"] = df_cflow["company_id"].astype(str).str.strip().str.upper()
        ...
        df_merged = pd.merge(df_fin, df_cflow[["company_id", "year_int", "fcf_conversion"]], on=["company_id", "year_int"], how="left")
    else:
...
def run_pros_cons_pipeline():
    ...
    try:
        ...
        df_companies, df_sectors, df_financials, df_cf, df_div, df_analysis = load_generator_data()
        
        # Sector mapping
        sectors_map = dict(zip(df_sectors["company_id"].str.strip().str.upper(), df_sectors["sector"]))
        ...
        print(f"CRITICAL: Pros/Cons Generator failed to load data: {e}", file=sys.stderr)
        return
    ...
    for idx, row in df_companies.iterrows():
        ...
        for rule in RULES_REGISTRY:
            ...
            if result:
                ...
                adjusted_score = score_sector_adjustment(result["confidence_pct"], rule.rule_id, sector)
    ...
    avg_confidence = round(total_confidence_sum / total_rules_triggered, 2) if total_rules_triggered > 0 else 0.0
    ...
    if companies_processed_count != 92:
        val_msg.append(f"Validation Warning: Processed {companies_processed_count} companies (expected 92).")
    ...
    if not df_out.empty:
        ...
        if min_conf < 0.0 or max_conf > 100.0:
            val_msg.append(f"Validation Fail: Confidence scores out of bounds [{min_conf}, {max_conf}].")
            validation_passed = False
    ...
    if len(registered_ids) != len(set(registered_ids)):
        val_msg.append(f"Validation Fail: Duplicate rule IDs detected in registry: {registered_ids}")
        validation_passed = False
        
    if validation_passed:
        pc_log.info("Validation passed successfully! Coverage: 100%. no duplicate rules, confidence within [0, 100].")
    else:
